...
def role_based_redirect(request):
    profile = request.user.profile  # Ensure a `Profile` is linked to each user
    if profile.role == 'driver':
        return redirect('driver_dashboard')  # Replace with your driver dashboard URL name
    elif profile.role == 'admin':
        return redirect('/admin_dashboard/')
    else:
        return redirect('logged_in')  # Replace with the regular user dashboard
...
class CustomLoginView(LoginView):
    def get_success_url(self):
        if self.request.user.groups.filter(name='Drivers').exists():
            return '/driver_dashboard/'  # URL for driver dashboard
        return '/logged_in/'  # URL for user dashboard
...
def add_announcement(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('announcement_list')  # Redirect to the announcement list
    else:
        form = AnnouncementForm()
    return render(request, 'admin/add_announcement.html', {'form': form})

# User side: View announcements
def announcement_list(request):
    announcements = Announcement.objects.all().order_by('-date_uploaded')
